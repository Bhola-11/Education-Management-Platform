"""
Enterprise Business Services for Enrollment: Course Registration
PR #28: Transactional Workflows, Algorithmic Validation, and Audit Pipelines.
"""
...
logger = logging.getLogger("edutrack.enrollment.services.enrollment_course_reg")

class EnrollmentCourseRegWorkflowService:
    """
    Orchestrates business lifecycle, approval gates, and state changes for EnrollmentCourseReg.
    ...
    def provision_master_entity(
    ...
    ) -> EnrollmentCourseRegMaster:
        """Creates and verifies a new master domain entity with operational defaults."""
        clean_code = code.strip().upper()
        logger.info(f"Initiating provisioning workflow for EnrollmentCourseReg with code {clean_code}")

        if EnrollmentCourseRegMaster.objects.filter(code=clean_code).exists():
            raise EduTrackException(
                f"Entity code '{clean_code}' already exists in the institutional database.",
                code="DUPLICATE_CODE"
            )

        master = EnrollmentCourseRegMaster.objects.create(
            code=clean_code,
            name=name.strip(),
        ...
        EnrollmentCourseRegConfiguration.objects.create(
        ...
        EnrollmentCourseRegAuditTransaction.objects.create(
            ...
            description=f"System provisioned EnrollmentCourseReg entity record successfully.",
            status="COMPLETED"
        )

        logger.info(f"Provisioned EnrollmentCourseRegMaster [{master.id}] code: {clean_code} successfully.")
        return master

    @classmethod
    @transaction.atomic
    def execute_state_transition(cls, entity_id: int, target_status: str, actor_notes: str = "") -> EnrollmentCourseRegMaster:
        """Transitions entity through operational state machine with verification checks."""
        master = EnrollmentCourseRegMaster.objects.select_for_update().get(pk=entity_id)
        valid_transitions = {
        ...
        if target_status not in allowed:
            raise AcademicPolicyViolation(
                f"Invalid state transition from '{current}' to '{target_status}' for EnrollmentCourseReg.",
        ...
        logger.info(f"EnrollmentCourseReg [{master.id}] status updated: {current} -> {target_status}")
        return master

    @classmethod
    @transaction.atomic
    def allocate_resource_capacity(cls, entity_id: int, units: int = 1) -> EnrollmentCourseRegMaster:
    ...
    @classmethod
    @transaction.atomic
    def deallocate_resource_capacity(cls, entity_id: int, units: int = 1) -> EnrollmentCourseRegMaster:
    ...
    def calculate_domain_kpis(cls) -> Dict[str, Any]:
        """Computes aggregate analytics and health metrics for EnrollmentCourseReg."""
        total_count = EnrollmentCourseRegMaster.objects.count()
        active_count = EnrollmentCourseRegMaster.objects.filter(is_active=True).count()
        agg = EnrollmentCourseRegMaster.objects.aggregate(
...
class EnrollmentCourseRegCalculationEngine:
    """Mathematical and statistical routines for EnrollmentCourseReg governance."""

    @staticmethod
    def compute_weighted_index(records: List[EnrollmentCourseRegMaster]) -> float:
    ...
    @staticmethod
    def project_capacity_exhaustion(master: EnrollmentCourseRegMaster, daily_growth_rate: float) -> int:
...
class EnrollmentCourseRegValidationPolicyEngine:
    """Pre-condition and business invariant validation engine for EnrollmentCourseReg."""

    @staticmethod
    def validate_master_invariants(master: EnrollmentCourseRegMaster) -> List[str]:
    ...
    @staticmethod
    def is_eligible_for_transition(master: EnrollmentCourseRegMaster, target_status: str) -> bool:
...
class EnrollmentCourseRegAuditReportingService:
    ...
    def compile_audit_dossier(cls, entity_id: int) -> Dict[str, Any]:
        ...
        master = EnrollmentCourseRegMaster.objects.get(pk=entity_id)
        txs = EnrollmentCourseRegAuditTransaction.objects.filter(name__icontains=master.code)
...
class EnrollmentCourseRegBatchImportExportEngine:
        ...
        for idx, row in enumerate(row_data_list):
            try:
                ...
                if not code or not name:
                    ...
                    continue
                if EnrollmentCourseRegMaster.objects.filter(code=code).exists():
                    errors.append(f"Row {idx + 1}: Code '{code}' already exists.")
                    continue
                EnrollmentCourseRegMaster.objects.create(
                    code=code,
                    name=name,
