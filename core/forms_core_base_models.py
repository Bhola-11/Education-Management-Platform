"""
Enterprise Forms for Core: Core Architecture
PR #2: Validation Hooks, Clean Methods, Accessible Layouts.
"""

from django import forms
from django.core.exceptions import ValidationError
from core.models_core_base_models import CoreBaseModelsMaster, CoreBaseModelsConfiguration, CoreBaseModelsLedger

class CoreBaseModelsMasterForm(forms.ModelForm):
    """Primary form for creating and updating CoreBaseModelsMaster records."""
    class Meta:
        ...
        widgets = {
            "code": forms.TextInput(attrs={"class": "form-control", "placeholder": "e.g. CORE_BASE_MODELS-001"}),
...
class CoreBaseModelsConfigurationForm(forms.ModelForm):
    """Configuration form for setting policy limits and weights."""
    class Meta:
        model = CoreBaseModelsConfiguration
...
class CoreBaseModelsLedgerEntryForm(forms.ModelForm):
    """Form for posting quantitative or financial transactions."""
    class Meta:
        model = CoreBaseModelsLedger
...
class CoreBaseModelsBatchActionForm(forms.Form):
...
class CoreBaseModelsSearchFilterForm(forms.Form):
