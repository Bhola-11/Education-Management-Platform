"""
Enterprise Business Services for Notifications: Notification Preferences
PR #79: Transactional Workflows, Algorithmic Validation, and Audit Pipelines.
"""
...
logger = logging.getLogger("edutrack.notifications.services.notifications_preferences")

class NotificationsPreferencesWorkflowService:
    """
    Orchestrates business lifecycle, approval gates, and state changes for NotificationsPreferences.
    ...
    def provision_master_entity(
    ...
    ) -> NotificationsPreferencesMaster:
        """Creates and verifies a new master domain entity with operational defaults."""
        clean_code = code.strip().upper()
        logger.info(f"Initiating provisioning workflow for NotificationsPreferences with code {clean_code}")

        if NotificationsPreferencesMaster.objects.filter(code=clean_code).exists():
            raise EduTrackException(
                f"Entity code '{clean_code}' already exists in the institutional database.",
                code="DUPLICATE_CODE"
            )

        master = NotificationsPreferencesMaster.objects.create(
            code=clean_code,
            name=name.strip(),
        ...
        NotificationsPreferencesConfiguration.objects.create(
        ...
        NotificationsPreferencesAuditTransaction.objects.create(
            ...
            description=f"System provisioned NotificationsPreferences entity record successfully.",
            status="COMPLETED"
        )

        logger.info(f"Provisioned NotificationsPreferencesMaster [{master.id}] code: {clean_code} successfully.")
        return master

    @classmethod
    @transaction.atomic
    def execute_state_transition(cls, entity_id: int, target_status: str, actor_notes: str = "") -> NotificationsPreferencesMaster:
        """Transitions entity through operational state machine with verification checks."""
        master = NotificationsPreferencesMaster.objects.select_for_update().get(pk=entity_id)
        valid_transitions = {
        ...
        if target_status not in allowed:
            raise AcademicPolicyViolation(
                f"Invalid state transition from '{current}' to '{target_status}' for NotificationsPreferences.",
        ...
        logger.info(f"NotificationsPreferences [{master.id}] status updated: {current} -> {target_status}")
        return master

    @classmethod
    @transaction.atomic
    def allocate_resource_capacity(cls, entity_id: int, units: int = 1) -> NotificationsPreferencesMaster:
    ...
    @classmethod
    @transaction.atomic
    def deallocate_resource_capacity(cls, entity_id: int, units: int = 1) -> NotificationsPreferencesMaster:
    ...
    def calculate_domain_kpis(cls) -> Dict[str, Any]:
        """Computes aggregate analytics and health metrics for NotificationsPreferences."""
        total_count = NotificationsPreferencesMaster.objects.count()
        active_count = NotificationsPreferencesMaster.objects.filter(is_active=True).count()
        agg = NotificationsPreferencesMaster.objects.aggregate(
...
class NotificationsPreferencesCalculationEngine:
    """Mathematical and statistical routines for NotificationsPreferences governance."""

    @staticmethod
    def compute_weighted_index(records: List[NotificationsPreferencesMaster]) -> float:
    ...
    @staticmethod
    def project_capacity_exhaustion(master: NotificationsPreferencesMaster, daily_growth_rate: float) -> int:
...
class NotificationsPreferencesValidationPolicyEngine:
    """Pre-condition and business invariant validation engine for NotificationsPreferences."""

    @staticmethod
    def validate_master_invariants(master: NotificationsPreferencesMaster) -> List[str]:
    ...
    @staticmethod
    def is_eligible_for_transition(master: NotificationsPreferencesMaster, target_status: str) -> bool:
...
class NotificationsPreferencesAuditReportingService:
    ...
    def compile_audit_dossier(cls, entity_id: int) -> Dict[str, Any]:
        ...
        master = NotificationsPreferencesMaster.objects.get(pk=entity_id)
        txs = NotificationsPreferencesAuditTransaction.objects.filter(name__icontains=master.code)
...
class NotificationsPreferencesBatchImportExportEngine:
        ...
        for idx, row in enumerate(row_data_list):
            try:
                ...
                if not code or not name:
                    ...
                    continue
                if NotificationsPreferencesMaster.objects.filter(code=code).exists():
                    errors.append(f"Row {idx + 1}: Code '{code}' already exists.")
                    continue
                NotificationsPreferencesMaster.objects.create(
                    code=code,
                    name=name,
